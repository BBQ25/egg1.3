...
const uint8_t cloudWeightRangeCount = 8;
CloudWeightRange cloudWeightRanges[cloudWeightRangeCount] = {
  {"reject", "Reject", 0.00f, 34.99f},
  {"peewee", "Peewee", 35.00f, 44.99f},
  {"pullet", "Pullet", 45.00f, 49.99f},
  {"small", "Small", 50.00f, 54.99f},
  {"medium", "Medium", 55.00f, 59.99f},
  {"large", "Large", 60.00f, 64.99f},
  {"extra_large", "Extra-Large", 65.00f, 69.99f},
  {"jumbo", "Jumbo", 70.00f, 1000.00f}
};

struct PendingCloudEvent {
  bool pending;
  uint8_t attempts;
...
// ======================================================================
const char* lcdIdleLine1 = "EggPulse";
...
String webCloudStatus   = "Booting";
String webCloudMessage  = "Waiting for WiFi";
String webCloudBatchCode = "None";
String webCloudLastSync = "Never";
...
unsigned long cloudSendSuccessCount = 0;
unsigned long cloudSendFailureCount = 0;
...
unsigned long lastRuntimeConfigFetchMs = 0;
...
bool cloudConfigLoaded = false;
bool cloudUploadsBlocked = false;
...
const uint8_t eggRecordCapacity = 20;
EggRecord eggRecords[eggRecordCapacity];
uint8_t eggRecordCount = 0;
uint8_t eggRecordNextIndex = 0;
unsigned long nextRecordId = 1;
...
void maintainCloudRuntime();
bool syncClockIfNeeded(bool forceSync = false);
...
String buildEggRecordsJSON();

void resetCounters();
void clearEggRecords();
...
String classifyCloudEgg(float weight);
...
String formatUptime();
String formatRecordTimestamp(unsigned long timestampMs);
String wifiQualityText(int rssi);
String statusClassFromText(String status);
String escapeJson(String text);
String buildCloudPayload(const PendingCloudEvent &event);
String buildIso8601Utc(time_t timestampSeconds);
...
void connectToWiFi() {
  ...
  webCloudStatus = "Connecting";
  webCloudMessage = "Waiting for WiFi";
...
void reconnectWiFiIfNeeded() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    webCloudStatus = "Offline";
    webCloudMessage = "WiFi disconnected";
  ...
  if (!wasWiFiConnected) {
    wasWiFiConnected = true;
    webCloudStatus = "Online";
    webCloudMessage = "WiFi restored";
    syncClockIfNeeded(true);
    fetchRuntimeConfig(true);
  }
}
...
void maintainCloudRuntime() {
  ...
  flushPendingCloudEvent(false);
}

bool syncClockIfNeeded(bool forceSync) {
  if (cloudClockSynced && !forceSync) {
    return true;
  }
  ...
  if (!forceSync && millis() - lastClockSyncAttemptMs < 30000UL) {
    return cloudClockSynced;
  }

  lastClockSyncAttemptMs = millis();
  configTime(0, 0, NTP_SERVER_PRIMARY, NTP_SERVER_SECONDARY);
  webCloudStatus = "Clock";
  webCloudMessage = "Syncing NTP time";
  ...
  cloudClockSynced = false;
  webCloudStatus = "Clock";
  webCloudMessage = "NTP sync failed";
  return false;
}

bool fetchRuntimeConfig(bool forceFetch) {
  if (cloudUploadsBlocked || WiFi.status() != WL_CONNECTED) {
    return false;
  }

  if (!forceFetch && cloudConfigLoaded && millis() - lastRuntimeConfigFetchMs < runtimeConfigRefreshMs) {
    return true;
  }
  ...
  if (httpCode == 401) {
    ...
    webCloudMessage = "Device credentials rejected";
    return false;
  }
  ...
  String serverTime = "";
  extractJsonStringValue(body, "server_time", serverTime);
  webCloudLastSync = serverTime.length() > 0 ? serverTime : formatUptime();

  bool batchWasNull = false;
  String batchCode = "";
  if (extractJsonStringValue(body, "open_batch_code", batchCode, &batchWasNull)) {
    webCloudBatchCode = batchWasNull ? "None" : batchCode;
  } else {
    webCloudBatchCode = "None";
  }

  for (uint8_t index = 0; index < cloudWeightRangeCount; index++) {
    applyWeightRangeFromConfig(body, cloudWeightRanges[index]);
  }

  cloudConfigLoaded = true;
  webCloudStatus = "Ready";
  webCloudMessage = webCloudBatchCode == "None"
    ? "Runtime config loaded"
    : "Batch " + webCloudBatchCode;
  return true;
}
...
bool flushPendingCloudEvent(bool forceAttempt) {
  ...
  if (httpCode == 422) {
    ...
    webCloudMessage = "Payload rejected";
    return false;
  }
...
String buildCloudPayload(const PendingCloudEvent &event) {
  String json = "{";
  json += "\"weight_grams\":" + String(event.weight_g, 2) + ",";
  json += "\"size_class\":\"" + escapeJson(event.cloudClass) + "\",";

  if (event.recordedAt.length() > 0) {
    json += "\"recorded_at\":\"" + escapeJson(event.recordedAt) + "\",";
  }

  if (event.batchCode.length() > 0) {
    json += "\"batch_code\":\"" + escapeJson(event.batchCode) + "\",";
  } else {
    json += "\"batch_code\":null,";
  }

  json += "\"egg_uid\":\"" + escapeJson(event.eggUid) + "\",";
  json += "\"metadata\":";
  json += deviceMetadataJson(event);
  json += "}";
  return json;
}
...
String deviceMetadataJson(const PendingCloudEvent &event) {
  String json = "{";
  json += "\"esp32_mac\":\"" + escapeJson(WiFi.macAddress()) + "\",";
  ...
  json += "\"measurement_span_g\":" + String(event.span_g, 2) + ",";
...
String buildIso8601Utc(time_t timestampSeconds) {
  if (timestampSeconds < 1700000000) {
    return "";
  }

  struct tm timeInfo;
  gmtime_r(&timestampSeconds, &timeInfo);
...
String deviceSerialTail() {
  ...
  for (size_t index = 0; index < strlen(DEVICE_SERIAL); index++) {
    char current = DEVICE_SERIAL[index];
  ...
  if (filtered.length() > 8) {
    filtered = filtered.substring(filtered.length() - 8);
  }

  return filtered.length() > 0 ? filtered : "device000";
}

String generateEggUid(time_t timestampSeconds) {
  unsigned long long epochMs = 0;
  if (timestampSeconds >= 1700000000) {
    epochMs = ((unsigned long long) timestampSeconds * 1000ULL) + (unsigned long long) (millis() % 1000UL);
  } else {
    epochMs = (unsigned long long) millis();
  }

  eggUidSequence++;
  char sequenceBuffer[5];
  snprintf(sequenceBuffer, sizeof(sequenceBuffer), "%04lu", eggUidSequence % 10000UL);
  char epochBuffer[32];
...
bool extractJsonStringValue(const String &json, const String &key, String &value, bool *wasNull) {
  ...
  int cursor = markerPos + marker.length();
  ...
  if (json.substring(cursor, cursor + 4) == "null") {
    value = "";
    if (wasNull != nullptr) {
      *wasNull = true;
    }
  ...
  value = json.substring(cursor, end);
  value.replace("\\\"", "\"");
  value.replace("\\\\", "\\");
  return true;
}
...
bool extractJsonNumberValue(const String &json, const String &key, float &value) {
  ...
  value = json.substring(cursor, end).toFloat();
  return true;
}

bool applyWeightRangeFromConfig(const String &json, CloudWeightRange &range) {
  String marker = "\"" + String(range.slug) + "\":";
  int markerPos = json.indexOf(marker);
  if (markerPos < 0) {
    return false;
  }

  int blockStart = json.indexOf("{", markerPos + marker.length());
  ...
  float minValue = range.minWeight;
  float maxValue = range.maxWeight;

  if (!extractJsonNumberValue(block, "min", minValue)) {
    return false;
  }

  if (!extractJsonNumberValue(block, "max", maxValue)) {
    return false;
  }

  range.minWeight = minValue;
  range.maxWeight = maxValue;
  return true;
}
...
void setupWebRoutes() {
  ...
  server.on("/records", HTTP_GET, []() {
    server.send(200, "application/json", buildEggRecordsJSON());
  });

  server.on("/reset", HTTP_GET, []() {
    resetCounters();
    server.send(200, "application/json", "{\"ok\":true,\"message\":\"Counters and records reset\"}");
  });
...
String buildJSONData() {
  ...
  json += "\"recordCount\":" + String(eggRecordCount) + ",";
...
String buildEggRecordsJSON() {
  String json = "{";
  json += "\"count\":" + String(eggRecordCount) + ",";
  json += "\"capacity\":" + String(eggRecordCapacity) + ",";
  json += "\"timestampSource\":\"uptime\",";
  json += "\"records\":[";

  for (uint8_t i = 0; i < eggRecordCount; i++) {
    uint8_t index = (eggRecordCount == eggRecordCapacity)
      ? (eggRecordNextIndex + i) % eggRecordCapacity
      : i;

    EggRecord &record = eggRecords[index];

    if (i > 0) json += ",";

    json += "{";
    json += "\"RecordID\":" + String(record.recordId) + ",";
    json += "\"Timestamp\":\"" + escapeJson(formatRecordTimestamp(record.timestampMs)) + "\",";
    json += "\"Weight_g\":" + String(record.weight_g, 1) + ",";
    json += "\"ClassLabel\":\"" + escapeJson(String(record.classLabel)) + "\",";
    json += "\"SortResult\":\"" + escapeJson(String(record.sortResult)) + "\",";
    json += "\"Notes\":\"" + escapeJson(String(record.notes)) + "\"";
    json += "}";
  }
...
void resetCounters() {
  ...
  totalFeedCycles = 0;
  totalReleased = 0;
  cloudSendSuccessCount = 0;
  cloudSendFailureCount = 0;
  cloudDroppedCount = 0;
  ...
  Serial.println("Counters and records reset");
}

void clearEggRecords() {
  eggRecordCount = 0;
  eggRecordNextIndex = 0;
  nextRecordId = 1;

  for (uint8_t i = 0; i < eggRecordCapacity; i++) {
    eggRecords[i].recordId = 0;
    eggRecords[i].timestampMs = 0;
    eggRecords[i].weight_g = 0.0;
    eggRecords[i].classLabel[0] = '\0';
    eggRecords[i].sortResult[0] = '\0';
    eggRecords[i].notes[0] = '\0';
  }
}
...
String classifyCloudEgg(float weight) {
  for (uint8_t index = 0; index < cloudWeightRangeCount; index++) {
    CloudWeightRange &range = cloudWeightRanges[index];
...
void processEggCycle() {
  ...
  String cloudEggType = classifyCloudEgg(finalWeight);
  ...
  Serial.print("Local Type   : "); Serial.println(eggType);
  Serial.print("Cloud Type   : "); Serial.println(cloudEggType);
  Serial.println("================================================");

  if (eggType != "Unknown") {
    addEggRecord(finalWeight, eggType, "Sorted", "Sent to " + eggType + " tray");
    updateCounters(eggType);
    queueCloudEvent(finalWeight, stats, eggType, cloudEggType, "Sorted");
    ...
    addEggRecord(finalWeight, eggType, "Rejected", "Weight out of range");
    countUnknown++;
    queueCloudEvent(finalWeight, stats, eggType, cloudEggType, "Rejected");
...
// ======================================================================
void addEggRecord(float weight, const String &classLabel, const String &sortResult, const String &notes) {
  EggRecord &record = eggRecords[eggRecordNextIndex];

  record.recordId = nextRecordId++;
  record.timestampMs = millis();
  record.weight_g = weight;

  copyTextToBuffer(record.classLabel, sizeof(record.classLabel), classLabel);
  copyTextToBuffer(record.sortResult, sizeof(record.sortResult), sortResult);
  copyTextToBuffer(record.notes, sizeof(record.notes), notes);

  eggRecordNextIndex = (eggRecordNextIndex + 1) % eggRecordCapacity;
  if (eggRecordCount < eggRecordCapacity) {
    eggRecordCount++;
  }
}
...
String formatRecordTimestamp(unsigned long timestampMs) {
  unsigned long elapsedMs = (timestampMs >= systemStartMillis)
    ? (timestampMs - systemStartMillis)
    : timestampMs;
