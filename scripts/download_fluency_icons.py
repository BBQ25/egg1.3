...
ALL_FLUENCY_URL = f"{BASE_URL}/icons/all--style-fluency"
SCRIPT_DIR = Path(__file__).resolve().parent
CATEGORY_INDEX_PATH = SCRIPT_DIR / "fluency_categories.json"
...
class Downloader:
    ...
    def get_fluency_categories(self) -> list[tuple[str, str]]:
        if CATEGORY_INDEX_PATH.exists():
            payload = json.loads(CATEGORY_INDEX_PATH.read_text(encoding="utf-8"))
            return self.filter_existing_categories([(str(item["href"]), str(item["label"])) for item in payload])

        html = self.fetch_html(ALL_FLUENCY_URL)
        soup = BeautifulSoup(html, "html.parser")
        categories: dict[str, str] = {}

        for anchor in soup.select('a[href*="--style-fluency"]'):
            href = anchor.get("href", "").strip()
            match = re.match(r"^/icons/set/([^/?#]+)--style-fluency$", href)
            if not match:
                continue
            slug = match.group(1)
        ...
        return self.filter_existing_categories(category_items)

    def category_exists(self, href: str) -> bool:
        url = BASE_URL + href
        try:
    ...
    def filter_existing_categories(self, categories: list[tuple[str, str]]) -> list[tuple[str, str]]:
        existing: list[tuple[str, str]] = []
        missing: list[str] = []

        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_category_workers * 2) as executor:
            future_map = {executor.submit(self.category_exists, href): (href, label) for href, label in categories}
            for future in concurrent.futures.as_completed(future_map):
                href, label = future_map[future]
                if future.result():
                    existing.append((href, label))
                else:
                    missing.append(href)

        self.stats["categories_missing"] = sorted(missing)
    ...
    def parse_category_page(self, category_href: str) -> tuple[str, str, list[IconRecord]]:
        ...
        slug_match = re.match(r"^/icons/set/([^/?#]+)--style-fluency$", category_href)
    ...
    def fetch_search_results(self, category_slug: str, category_name: str) -> list[IconRecord]:
        ...
        for offset in range(0, 10000, 60):
            url = (
                ...
                f"?style=fluency&language=en&analytics=true&spellcheck=false&amount=60&isOuch=true"
    ...
    def fetch_category_api(self, category_code: str) -> dict[str, Any]:
        ...
        for offset in range(0, 10000, 100):
            url = (
                ...
                f"?amount=100&offset={offset}&style=fluency&language=en-US&category={category_code}"
            )
            response = self.session().get(url, timeout=self.request_timeout)
    ...
    def build_paths(self, record: IconRecord, icons_dir: Path, animated_dir: Path) -> tuple[Path, Path | None]:
        platform_tag = self.sanitize_filename(record.icon_id)
        base_name = self.sanitize_filename(f"icons8-{record.common_name}-{platform_tag}")
    ...
    def process_category(self, category_href: str) -> dict[str, Any]:
        ...
        for record in records:
            png_url = f"https://img.icons8.com/?size=100&id={record.icon_id}&format=png"
            gif_url = f"https://img.icons8.com/?size=100&id={record.icon_id}&format=gif" if record.is_animated else None
    ...
    def run(self) -> None:
        ...
        categories = self.get_fluency_categories()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Download all Icons8 Fluency icon categories into structured folders.")
    parser.add_argument(
        "--root",
        default="resources/icons/Fluency All",
        help="Destination root folder for downloaded Fluency icons.",
    )
    parser.add_argument(
        "--max-workers",
        type=int,
