...
ALL_MACOS_URL = f"{BASE_URL}/icons/all--os-macos"
SCRIPT_DIR = Path(__file__).resolve().parent
CATEGORY_INDEX_PATH = SCRIPT_DIR / "macos_categories.json"
...
class Downloader:
    ...
    def get_macos_categories(self) -> list[tuple[str, str]]:
        ...
        html = self.fetch_html(ALL_MACOS_URL)
        soup = BeautifulSoup(html, "html.parser")
        categories: dict[str, str] = {}

        for anchor in soup.select('a[href*="--os-macos"]'):
            href = anchor.get("href", "").strip()
            match = re.match(r"^/icons/set/([^/?#]+)--os-macos$", href)
            if not match:
                continue
            slug = match.group(1)
    ...
    def parse_category_page(self, category_href: str) -> tuple[str, str, list[IconRecord]]:
        ...
        slug_match = re.match(r"^/icons/set/([^/?#]+)--os-macos$", category_href)
    ...
    def fetch_search_results(self, category_slug: str, category_name: str) -> list[IconRecord]:
        ...
        for offset in range(0, 10000, 60):
            url = (
                ...
                f"?platform=macos&language=en&analytics=true&spellcheck=false&amount=60"
    ...
    def fetch_category_api(self, category_code: str) -> dict[str, Any]:
        ...
        for offset in range(0, 10000, 100):
            url = (
                ...
                f"?amount=100&offset={offset}&platform=macos&language=en-US&category={category_code}"
            )
            response = self.session().get(url, timeout=self.request_timeout)
    ...
    def run(self) -> None:
        ...
        categories = self.get_macos_categories()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Download all Icons8 macOS icon categories into structured folders.")
    parser.add_argument(
        "--root",
        default="resources/icons/macOS All",
        help="Destination root folder for downloaded macOS icons.",
    )
    parser.add_argument(
        "--max-workers",
        type=int,
