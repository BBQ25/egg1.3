...
ALL_WINDOWS_URL = f"{BASE_URL}/icons/all--os-windows"
SCRIPT_DIR = Path(__file__).resolve().parent
CATEGORY_INDEX_PATH = SCRIPT_DIR / "windows_categories.json"
...
class Downloader:
    ...
    def get_windows_categories(self) -> list[tuple[str, str]]:
        ...
        html = self.fetch_html(ALL_WINDOWS_URL)
        soup = BeautifulSoup(html, "html.parser")
        categories: dict[str, str] = {}

        for anchor in soup.select('a[href*="--os-windows"]'):
            href = anchor.get("href", "").strip()
            match = re.match(r"^/icons/set/([^/?#]+)--os-windows$", href)
            if not match:
                continue
            slug = match.group(1)
    ...
    def parse_category_page(self, category_href: str) -> tuple[str, str, list[IconRecord]]:
        ...
        slug_match = re.match(r"^/icons/set/([^/?#]+)--os-windows$", category_href)
    ...
    def fetch_search_results(self, category_slug: str, category_name: str) -> list[IconRecord]:
        ...
        for offset in range(0, 10000, 60):
            url = (
                ...
                f"?platform=windows&language=en&analytics=true&spellcheck=false&amount=60"
    ...
    def fetch_category_api(self, category_code: str) -> dict[str, Any]:
        ...
        for offset in range(0, 10000, 100):
            url = (
                ...
                f"?amount=100&offset={offset}&platform=windows&language=en-US&category={category_code}"
            )
            response = self.session().get(url, timeout=self.request_timeout)
    ...
    def run(self) -> None:
        ...
        categories = self.get_windows_categories()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Download all Icons8 Windows icon categories into structured folders.")
    parser.add_argument(
        "--root",
        default="resources/icons/Windows All",
        help="Destination root folder for downloaded Windows icons.",
    )
    parser.add_argument(
        "--max-workers",
        type=int,
